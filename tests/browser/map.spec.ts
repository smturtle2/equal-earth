...
import { fitScale, invert } from '../../src/projection';
...
function referencePixel(x: number, y: number, width: number, height: number, attitude: Attitude): number[] {
  ...
  for (let sy = 0; sy < 4; sy++) for (let sx = 0; sx < 4; sx++) {
    const point = invert((x + (sx + 0.5) / 4 - width / 2) / scale, (height / 2 - y - (sy + 0.5) / 4) / scale);
    let color = [1, 1, 1];
    if (point) {
      const [lon, lat] = point;
...
async function installReadback(page: Page) {
  await page.addInitScript(() => {
    const stats = { submits: 0, contexts: [] as string[] };
    ...
    Object.assign(window, { mapTestStats: stats, readMap: async () => {
      ...
      for (const u of [0.05, 0.25, 0.5, 0.75, 0.95]) for (const v of [0.149, 0.15, 0.151, 0.155, 0.16, 0.175, 0.25, 0.5, 0.75, 0.825, 0.84, 0.849, 0.85, 0.851]) {
...
test('maps a known raster accurately through rotation, handles input and idles', async ({ page }) => {
  ...
  await expect(page.locator('button, nav, header')).toHaveCount(0);
  ...
  expect((await stats()).contexts).toEqual(['webgpu']);
...
test('loads both real textures, preserves the view, and recovers from failed or superseded switches', async ({ page }) => {
  ...
  await page.goto('/');
  const canvas = page.locator('#map');
  const selector = page.getByLabel('지도 텍스처');
  await expect(canvas).toHaveAttribute('data-state', 'ready');
  await expect(canvas).toHaveAttribute('data-texture', 'natural-earth');
  const pixels = () => page.evaluate(() => (window as unknown as {
    readMap: () => Promise<{ hash: number; white: number }>
  }).readMap());
  await canvas.press('Shift+ArrowDown');
  await canvas.press('+');
  await page.waitForTimeout(150);
  const original = await pixels();
  await selector.selectOption('blue-marble');
  ...
  await selector.selectOption('natural-earth');
  await expect(canvas).toHaveAttribute('data-texture', 'natural-earth');
  await page.waitForTimeout(100);
  ...
  await expect(selector).toHaveValue('natural-earth');
  expect((await pixels()).hash).toBe(original.hash);
