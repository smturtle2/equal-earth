  ...
  for (const { canvas, zoom, radiusRatio = 0.6 } of surfaces) {
    ...
    canvas.addEventListener('pointermove', (event) => {
      ...
      if (pointers.size === 1) {
        ...
        else motion.drag(previous, next, width, height, now, radiusRatio);
        ...
        motion.drag(a.middle, b.middle, width, height, now, radiusRatio);
        const angle = a.angle - b.angle;
        motion.rotate([0, 0, 1], Math.atan2(Math.sin(angle), Math.cos(angle)), now);
        if (zoom && a.length > 5 && b.length > 5) motion.magnify(b.length / a.length);
      }
      invalidate();
    });
